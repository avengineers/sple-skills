...
This skill owns the name of a **unit** test file. `c-integration-testing` owns the integration one.
The markers make the convention checkable — `test/skills/test_skill_structure.py` fails when any
skill document shows a test file name that matches no declared convention.

<!-- test-file-naming:begin -->

| Level | File name |
|-------|-----------|
| Unit | `test_*.cc` — the `test_` prefix and the `.cc` extension are binding |

<!-- test-file-naming:end -->

One prefix and one extension for every test file, so a single `test_*` glob finds them all and
tests sort together in a directory listing.
