...
> **RULE**: If the staged changes touch **only test files** (e.g. `test_*.cc`, files under `test/`), the type is **always `test:`** — regardless of whether the change adds tests, refactors logic, cleans up style, replaces magic numbers, or updates comments. Never use `style:`, `refactor:`, or `chore:` for changes limited to test files.
...
└── NO: Did you change ONLY test files (test_*.cc, files under test/)?
