...
This skill owns the name of an **integration** test file; `c-unit-testing` owns the unit one. Same
prefix and same extension as a unit test, with the level as a suffix — so one `test_*` glob finds
every test in the project and tests sort together.

<!-- test-file-naming:begin -->

| Level | File name |
|-------|-----------|
| Integration | `test_*_integration.cc` — the `test_` prefix, the `_integration` suffix and the `.cc` extension are binding |

<!-- test-file-naming:end -->

```text
test/
├── integration/
│   ├── test_subsystem_a_integration.cc
│   ├── test_subsystem_b_integration.cc
│   └── test_end_to_end_integration.cc
└── <variant>/
...
add_executable(test_subsystem_integration
    ${COMPONENT_SOURCES}
    test/test_subsystem_integration.cc
